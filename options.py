...
class OptionManager:
    ...
    def __init__(self):
        globs.log.write(1, function='Options', action='OptionManager', msg='Initializing option manager.')
        return None
    ...
    def openRcFile(self, rcFileSpec):
        globs.log.write(1, function='Options', action='openRcFile', msg='Opening .rc file {}'.format(globs.maskData(rcFileSpec, self.maskPath())))
        if self.rcFileName:     # Rc file already initiailzed. Something is wrong.
            globs.log.write(1, function='Options', action='openRcFile', msg='RC file {} already initialized. {} is a duplicate request.'.format(self.rcFileName, globs.maskData(rcFileSpec, self.maskPath())))
            return False
        try:
            self.parser = configparser.SafeConfigParser(interpolation=None)
            self.parser.read(rcFileSpec)
        except configparser.ParsingError as err:
            globs.log.write(1, function='Options', action='openRcFile', msg='RC file parsing error: {} {}\n'.format(globs.maskData(rcFileSpec, self.maskPath()), err))
            return False
    ...
    def checkRcFileVersion(self):
        ...
        if needToUpgrade == False:
            ...
            globs.log.write(3, function='Options', action='checkRcFileVersion', msg='RC file versions: current={} new={}.'.format(currVerNum, newVerNum))
            if currVerNum < newVerNum: # .rc file need an upgrade
                needToUpgrade = True

        globs.log.write(1, function='Options', action='checkRcFileVersion', msg='Current version number={}. Need to upgrade rc file? {}'.format(currVerNum, needToUpgrade))
    ...
    def setRcDefaults(self):
        globs.log.write(1, function='Options', action='setRcDefaults', msg='Setting .rc file defaults')
        errlist = []

        if not self.parser:
            globs.log.write(1, function='Options', action='setRcDefaults', msg='RC file not yet opened. Can not set defaults.')
            return False
        ...
        for section, option, default, canCont in rcParts:
            ...
            if not self.parser.has_section(section): # Whole section is missing.
                globs.log.write(3, function='Options', action='setRcDefaults', msg='Adding RC section: [{}]'.format(section))
                self.parser.add_section(section)
                needUpdate = True

            if not self.parser.has_option(section, option): # Option is missing. Might be able to continue if non-critical.
                globs.log.write(3, function='Options', action='setRcDefaults', msg='Adding RC option: [{}] {}={}'.format(section, option, default))
        ...
        globs.log.write(3, function='Options', action='setRcDefaults', msg='needUpdate = {} defaultsOK={}'.format(needUpdate, defaultsOK))
        if needUpdate:
            self.updateRc()
        if not defaultsOK:
            for i in range(len(errlist)):
                globs.log.err(errlist[i])
                globs.log.write(1, function='Options', action='setRcDefaults', msg=errlist[i])
        return defaultsOK
    ...
    def readRcOptions(self):
        globs.log.write(1, function='Options', action='readRcOptions', msg='Reading .rc file options')
        restart = False
        ...
        if self.options['dateformat'] not in drdatetime.dtFmtDefs:
            globs.log.write(1, function='Options', action='readRcOptions', msg='RC file error: Invalid date format: [{}]\n'.format(self.options['dateformat']))
            restart = True

        # Check for valid time format
        if self.options['timeformat'] not in drdatetime.dtFmtDefs:
            globs.log.write(1, function='Options', action='readRcOptions', msg='RC file error: Invalid time format [{}]\n'.format(self.options['timeformat']))
            restart = True
        ...
        for option, optName, globName in configList:
            if option != None:  # option specified on command line
                globs.log.write(3, function='Options', action='readRcOptions', msg='Option {} specified on command line.'.format(optName))
        ...
        for rb in ['rollback', 'rollbackx']:
            if self.options[rb] != None: # Roll back and continue
                if not drdatetime.toTimestamp(self.options[rb], self.options['dateformat'], self.options['timeformat']):
                    globs.log.write(1, function='Options', action='readRcOptions', msg='Invalid rollback date specification: {}.\n'.format(self.options[rb]))
        ...
        for opName in self.options:
            if opName in ('rcfilename', 'dbpath', 'logpath', 'inserver', 'inaccount', 'inpassword', 'outserver', 'outaccount', 'outpassword', 'outsender', 'outsendername', 'outreceiver'): # Mask sensitive data fields
                globs.log.write(3, function='Options', action='readRcOptions', msg='Parsed config option [{}]=[{}]'.format(opName, globs.maskData(self.options[opName], self.options['masksensitive'])))
            else:
                globs.log.write(3, function='Options', action='readRcOptions', msg='Parsed config option [{}]=[{}]'.format(opName, self.options[opName]))

        globs.log.write(1, function='Options', action='readRcOptions', msg='Need to restart? {}'.format(restart))
        return restart


    # save updated RC configuration to .rc file
    def updateRc(self):
        globs.log.write(1, function='Options', action='updateRc', msg='Updating .rc file')
    ...
    def processCmdLineArgs(self):
        globs.log.write(1,  function='Options', action='processCmdLineArgs', msg='Processing command line arguments.')
        ...
        try:
            ...
            e = sys.exc_info()[0]
            globs.log.write(1, function='Options', action='readRcOptions', msg='Argument parser error: {}'.format(e))
            globs.closeEverythingAndExit(1)

        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='rcpath = [{}]'.format(globs.maskData(self.cmdLineArgs.rcpath, True)))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='dbpath = [{}]'.format(globs.maskData(self.cmdLineArgs.dbpath, True)))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='logpath = [{}]'.format(globs.maskData(self.cmdLineArgs.logpath, True)))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='verbose = [{}]'.format(self.cmdLineArgs.verbose))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='Version = [{}]'.format(self.cmdLineArgs.Version))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='append = [{}]'.format(self.cmdLineArgs.append))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='size = [{}]'.format(self.cmdLineArgs.size))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='initdb = [{}]'.format(self.cmdLineArgs.initdb))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='rollback = [{}]'.format(self.cmdLineArgs.rollback))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='rollbackx = [{}]'.format(self.cmdLineArgs.rollbackx))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='file = [{}]'.format(self.cmdLineArgs.file))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='fileattach = [{}]'.format(self.cmdLineArgs.fileattach))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='nomail = [{}]'.format(self.cmdLineArgs.nomail))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='remove = [{}]'.format(self.cmdLineArgs.remove))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='purgedb = [{}]'.format(self.cmdLineArgs.purgedb))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='stopbackupwarn = [{}]'.format(self.cmdLineArgs.stopbackupwarn))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='collect = [{}]'.format(self.cmdLineArgs.collect))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='report = [{}]'.format(self.cmdLineArgs.report))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='masksensitive = [{}]'.format(self.cmdLineArgs.masksensitive))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='nomasksensitive = [{}]'.format(self.cmdLineArgs.nomasksensitive))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='validatereport = [{}]'.format(self.cmdLineArgs.validatereport))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='layout = [{}]'.format(self.cmdLineArgs.layout))
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='emailservers = [{}]'.format(self.cmdLineArgs.emailservers))
    
        # Figure out where RC file is located
        if self.cmdLineArgs.rcpath is not None:  # RC Path specified on command line
            globs.log.write(3, function='Options', action='processCmdLineArgs', msg='RC path specified on command line.')
            ...
            globs.log.write(3, function='Options', action='processCmdLineArgs', msg='RC path not specified on command line. Using default.')
            rc = '{}/{}'.format(globs.progPath, globs.rcName)
        self.options['rcfilename'] = rc
        
        globs.log.write(3, function='Options', action='processCmdLineArgs', msg='Final RC path=[{}]'.format(globs.maskData(self.options['rcfilename'], self.maskPath())))
        return None

    # Get individual .rc file option
    def getRcOption(self, section, option):
        globs.log.write(1, function='Options', action='getRcOption', msg='Retrieving .rc option: [{}]{}='.format(section, option))
    ...
    def setRcOption(self, section, option, value):
        globs.log.write(1, function='Options', action='setRcOption', msg='Setting .rc option: [{}]{}={}'.format(section, option, value))
        self.parser.set(section, option, value)
        return None

    # Clear individual option in .rc file
    def clearRcOption(self, section, option):
        globs.log.write(1, function='Options', action='clearRcOption', msg='Deleting .rc option: [{}]{}='.format(section, option))
        self.parser.remove_option(section, option)
        return None

    # Add a new section to the .rc file
    def addRcSection(self, section):
        globs.log.write(1, function='Options', action='addRcSection', msg='Adding .rc section: [{}]'.format(section))
        self.parser.add_section(section)
        return None

    def getRcSection(self, section):
        globs.log.write(1, function='Options', action='getRcSection', msg='Retrieving .rc section: [{}]'.format(section))
        vals = {}
    ...
    def hasSection(self, section):
        globs.log.write(1, function='Options', action='hasSection', msg='Check if .rc file has [{}] section'.format(section))

        retval=False
        if self.parser.has_section(section):
            retval=True
        globs.log.write(3, function='Options', action='hasSection', msg='Does section [{}] exist? {}'.format(section, retval))
        return retval


    def clearRcSection(self, section):
        globs.log.write(3, function='Options', action='clearRcSection', msg='Deleting .rc section [{}]'.format(section))
        self.parser.remove_section(section)
        return None

    def getRcSectionDateTimeFmt(self, src, dest):
        globs.log.write(3, function='Options', action='getRcSectionDateTimeFmt', msg='Getting Date/Time format for .rc section [{}{}{}]'.format(src, globs.opts['srcdestdelimiter'],dest))
        ...
        if self.parser.has_option(section, 'dateformat'):
            ...
            if tmp in drdatetime.dtFmtDefs: # Is it a legal date format?
                ...
                globs.log.write(1, function='Options', action='getRcSectionDateTimeFmt', msg='Invalid date format in .rc file, [{}{}{}]: dateformat={}'.format(src, globs.opts['srcdestdelimiter'], dest, tmp))
        ...
        if self.parser.has_option(section, 'timeformat'):
            ...
            if tmp in drdatetime.dtFmtDefs: # Is it a legal time format?
                ...
                globs.log.write(1, function='Options', action='getRcSectionDateTimeFmt', msg='Invalid time format in .rc file, [{}{}{}]: dateformat={}'.format(src, globs.opts['srcdestdelimiter'], dest, tmp))

        globs.log.write(3, function='Options', action='getRcSectionDateTimeFmt', msg='Correct date format=[{}]  Correct time format=[{}]'.format(dtfmt, tmfmt))
        return dtfmt, tmfmt
...
def initOptions():
    globs.log.write(1, function='Options', action='initOptions', msg='Initializing global program options.')
    ...
    globs.log.write(1, function='Options', action='initOptions', msg='Processing command line.')
    ...
    if needToUpgrade is True and os.path.isfile(oMgr.options['rcfilename']):
        globs.log.write(1, function='Options', action='initOptions', msg='RC file is out of date. Needs update from version {} to version {}{}{}.'.format(currRcVersion, globs.rcVersion[0], globs.rcVersion[1], globs.rcVersion[2]))
        import convert
        convert.convertRc(oMgr, currRcVersion)
        globs.log.write(1, function='Options', action='initOptions', msg='RC file has been updated to the latest version.')
    ...
    if oMgr.setRcDefaults() is False:
        globs.log.write(1, function='Options', action='initOptions', msg='RC file {} has changed or has an unrecoverable error. Please edit file with proper configuration, then re-run program'.format(oMgr.options['rcfilename']))
        return False
   ...
   # If output files are specified on the command line (-f), make sure their specification is correct
    ...
    globs.log.write(1, function='Options', action='initOptions', msg='Option initialization complete. Continuing program.')
    return True
...
def validateOutputFiles():
    ...
    if globs.ofileList:    # Potential list of output files specified on command line
        for fspec in globs.ofileList:
            ...
            if len(fsplit) != 2:
                globs.log.write(1, function='Options', action='validateOutputFiles', msg='Invalid output file specificaton: {}. Correct format is <filespec>,<format>. Please check your command line parameters.'.format(fsplit))
                canContinue = False
            elif fsplit[1] not in ('html','txt', 'csv', 'json'):
                globs.log.write(1, function='Options', action='validateOutputFiles', msg='Output file {}: Invalid output file format specificaton: {}. Please check your command line parameters.'.format(fsplit[0], fsplit[1]))
                canContinue = False
